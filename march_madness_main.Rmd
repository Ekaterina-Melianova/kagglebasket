...
TODO: Plot one network as an example
...
# 1. Example of the network with Highest PageRank centralization
...
```{r Plot the heatmap with Conferences and Seasons competitiveness}

heatmap_comp <- spread(df_conferences_stats_seasons, season, eigenvector_centralization)
rownames(heatmap_comp) <- heatmap_comp$conference_name
heatmap_comp <- as.matrix(heatmap_comp %>% select(-c(conference_name)))


pheatmap(heatmap_comp, cluster_cols = F, angle_col=45, main="Competitiveness in Conferences for the past 20 years")
...
# TODO: Add color according to some advanced statistic of the team as an example.

# Plot the network
vertex_size <- as.numeric(strength(g_teams_full, mode="in")) * 0.2
plot.igraph(g_teams_full, edge.arrow.size=0.05, layout=layout.fruchterman.reingold, edge.width=0.01,
...
```{r Model 1. Edges + Mutality + nodecov}

ergm_model.1 <- ergm(g_teams_full_net ~ mutual +
                        nodecov("TSPerc") + 
                        nodecov("AstRatio") +
                        nodecov("TORatio") + 
                        nodecov("ThreesShare") + 
...
summary(ergm_model.1)

```

```{r Model 2. Edges + Mutality + nodecov + absdiff}

ergm_model.2 <- ergm(g_teams_full_net ~ mutual +
                        nodecov("TSPerc") + 
                        nodecov("AstRatio") +
                        nodecov("TORatio") + 
                        nodecov("ThreesShare") + 
...
summary(ergm_model.2)

```

```{r Model 3. Edges + Mutality + nodecov + diff}

ergm_model.3 <- ergm(g_teams_full_net ~ mutual +
                        nodecov("TSPerc") + 
                        nodecov("AstRatio") +
                        nodecov("TORatio") + 
                        nodecov("ThreesShare") + 
...
summary(ergm_model.3)

```


```{r Model 4. Mutality + nodeicov + diff}

ergm_model.4 <- ergm(g_teams_full_net ~ mutual + 
                        nodeicov("TSPerc") + 
                        nodeicov("AstRatio") +
                        nodeicov("TORatio") + 
...
ergm_model.5 <- ergm(g_teams_full_net ~ edges + mutual + 
                        nodeicov("TSPerc") + 
                        nodeicov("AstRatio") +
                        nodeicov("TORatio") + 
                        nodeicov("ThreesShare") + 
